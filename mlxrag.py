...
def run_indexing(args):
    ...
    db_type = getattr(args, 'db_type', 'qdrant')
    ...
    if not processor_args.get("use_ollama", False) and hasattr(args, 'use_mlx_models') and HAS_MLX_EMBEDDING_MODELS:
        processor_args.update({
    ...
    try:
        ...
        if db_type.lower() == 'qdrant':
            ...
            db_args.update({
                "uri": getattr(args, 'lancedb_uri', None),
            })
        elif db_type.lower() == 'meilisearch':
            db_args.update({
                "url": getattr(args, 'meilisearch_url', "http://localhost:7700"),
                "api_key": getattr(args, 'meilisearch_api_key', None),
            })
        elif db_type.lower() == 'elasticsearch':
            db_args.update({
                "hosts": getattr(args, 'es_hosts', ["http://localhost:9200"]),
                "api_key": getattr(args, 'es_api_key', None),
                "username": getattr(args, 'es_username', None),
